...
def add_to_cart_view(request,pk):
    ...
    response = render(request, 'customer_home.html',{'products':products,'product_count_in_cart':product_count_in_cart})
...
def ProductDetail(request,pk):
    products=models.Product.objects.get(pk=pk)
    if 'product_ids' in request.COOKIES:
        product_ids = request.COOKIES['product_ids']
        counter=product_ids.split('|')
        product_count_in_cart=len(set(counter))
...
def profile(request):
    ...
    data=models.Customer.objects.filter(username=user)
    return render(request,'my-profile.html',{'data':data})
